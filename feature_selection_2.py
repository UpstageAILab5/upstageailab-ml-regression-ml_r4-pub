...
def auto_label_encode(df, cols_exclude, encoder_type = 'onehot'):
    """
    ...
    for column in tqdm(df_transformed.columns):
        ...
        if df_transformed[column].dtype == 'object' or df_transformed[column].dtype.name == 'category':
            ...
            encoded = encoder.fit_transform(df_transformed[[column]])  # 2D로 변환
            encoded_df = pd.DataFrame(encoded, columns=encoder.get_feature_names_out([column]))
            df_transformed = pd.concat([df_transformed.drop(columns=[column]), encoded_df], axis=1)
    print(df.shape, df_transformed.shape)
    # return pd.concat([df_rest, df_transformed], axis=1).sort_index(axis=1)
    return pd.concat([df[cols_exclude], df_transformed], axis=1).sort_index(axis=1)
class Utils:
    @staticmethod
...
## Encode categorical variables
    ...
    df_transformed = auto_label_encode(df, cols_exclude, 'onehot')
    print("Original DataFrame:\n", df.shape)
    print("\nTransformed DataFrame:\n", df_transformed.shape)
    df_transformed.to_csv(os.path.join(prep_path, 'df_combined_distance_feature_after_null_fill_encoded.csv'))
...
## Select Features
    ...
    df_combined = pd.read_csv(os.path.join(prep_path, 'df_combined_distance_feature_after_null_fill.csv'))
