...
class DataPrep():
    ...
    def data_prep(self, remove_cols: List[str] = None):
        ####
        print('#### Data Prep starts...\n Load Data.')
        concat = self._load_data_concat_train_test()
        print(f'Data prep 시작 전 데이터 shape: {concat.shape}')
    ...
    def _load_data_concat_train_test(self):
        try:
            ...
            print(f'train/test 구분을 위한 칼럼을 하나 만들어 줍니다. Train data shape : {dt.shape}, Test data shape : {dt_test.shape}\n')
            # Train과 Test data를 살펴보겠습니다.
    
            dt['is_test'] = 0
            dt_test['is_test'] = 1
            print('is_test column added to train and test data.\nConcat train and test data.')
            concat = pd.concat([dt, dt_test])     # 하나의 데이터로 만들어줍니다.
            concat['is_test'].value_counts()      # train과 test data가 하나로 합쳐진 것을 확인할 수 있습니다.
            print('칼럼 이름을 쉽게 바꿔주겠습니다. 다른 칼럼도 사용에 따라 바꿔주셔도 됩니다!')
            concat = concat.rename(columns={'전용면적(㎡)':'전용면적'})
            print(f'Concat data shape : {concat.shape}\n')
            print(f'Column name converted from 전용면적(㎡) to 전용면적 : {concat.columns}')
            print('#### End Data Prep.. ')
            return concat
    ...
    def _null_allocation(self, concat, remove_cols: List[str] = None):
        ...
        print('### Null Prep. 아무 의미도 갖지 않는 칼럼은 결측치와 같은 역할을 하므로, np.nan으로 채워 결측치로 인식되도록 합니다.')
        if remove_cols:
            for col in remove_cols:
                print(f'{col} column nan 처리')
    ...
    def _remove_null(self, concat):
        ...
        # 로깅
        print(f'* 결측치 비율이 {self.thr_ratio_null} 이하인 변수들: {list(columns_to_keep)}')
        print(f'* 결측치 비율이 {self.thr_ratio_null} 초과인 변수들: {list(columns_to_drop)}')
    ...
    def _interpolation(self, concat: pd.DataFrame) -> pd.DataFrame:
        ...
        print(f"연속형 변수: {numerical_columns}")
        print(f"범주형 변수: {categorical_columns}")
        
        group_cols = ['시군구', '도로명', '아파트명']
        print('범주형 변수 결측치 처리')
        print(concat_select[categorical_columns].isnull().sum())
        # 범주형 변수 결측치 처리
        ...
        print('수치형 변수 결측치 처리')
        print(concat_select[numerical_columns].isnull().sum())
        # 수치형 변수 결측치 처리
    ...
    def remove_outliers_iqr(self,dt, column_name):
        ...
        print(f'{column_name} 이상치 제거 후 데이터 shape: {dt.shape}')
        return result